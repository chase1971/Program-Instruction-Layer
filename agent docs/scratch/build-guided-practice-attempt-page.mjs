/**
 * Generator for guided-practice attempt review HTML (sections A–D).
 *
 * Run:
 *   node "agent docs/scratch/build-guided-practice-attempt-page.mjs"
 *   node "agent docs/scratch/build-guided-practice-attempt-page.mjs" --payload attempt-6f9a7f87-payload.json --student "Michelle (Spare 3)"
 */
...
const payloadFile = args.payload ?? 'attempt-60b0a608-payload.json';
const studentLabel = args.student ?? 'Instructor test session';
const payloadRows = JSON.parse(fs.readFileSync(path.join(dir, payloadFile), 'utf8'));
const payload = Array.isArray(payloadRows) ? payloadRows[0] : payloadRows;
if (!payload?.id) {
  throw new Error(`No attempt row in ${payloadFile}`);
}

const slug = args.slug ?? payload.id.slice(0, 8);
const htmlName = `guided-practice-attempt-${slug}.html`;
const eventCount = Array.isArray(payload.session_log?.events) ? payload.session_log.events.length : 0;
...
<title>Guided practice attempt — ${studentLabel} — problem ${payload.problem_id}</title>
<style>
  :root {
    --ink: #13243a;
    --soft: #5a6b80;
...
<div class="banner">
  <strong>Scratch prototype — live Supabase data.</strong>
  ${studentLabel} · guided practice problem ${payload.problem_id} · ${payload.score}/${payload.max_score} · ${eventCount} events.
  Four display sections (A–D) — click any question tile to open details and add notes.
</div>
...
<nav class="jump" aria-label="Jump to section">
  ...
  <a href="#decoded-timeline">C. Decoded timeline <span>All ${eventCount} events with meta shown</span></a>
...
<section class="section-card" id="decoded-timeline" data-comment-id="section-decoded-timeline" data-comment-title="Section C — Decoded timeline layout">
  ...
  <p class="secnote">All ${eventCount} events with <code>meta</code> — collapsed by default.</p>
  <button type="button" class="collapsible-toggle" id="timeline-toggle" aria-expanded="false">Show decoded timeline (${eventCount} events)</button>
...
(function () {
  ...
  document.getElementById('hero-sub').textContent =
    'Attempt ' + DATA.attemptId.slice(0, 8) + '… · ${studentLabel.replace(/'/g, "\\'")} · ' + events.length + ' events';
...
(function () {
  'use strict';
  var PAGE = 'scratch/${htmlName}';
  var DATA_URL = '/scratch/guided-practice-attempt-${slug}.comments.json';
