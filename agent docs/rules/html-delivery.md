...
**`agent docs/session-tracking-log.html`** — one row per task bump: cost, model, docs that failed to route.  
**`agent docs/session-metrics-log.html`** — one card per finalized session: cost, tasks, notes.

Both generated by `scripts/append-session-scorecard.js`. Append via `--note` / `--finalize-file` — **never read these HTML files into context** (Chase's dashboard).  
