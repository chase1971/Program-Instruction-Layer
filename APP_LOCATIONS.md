...
| **Manim**, division animation trial | `Manim Trial` | *(local experiment)* | Isolated Python animation environment; setup and headless rendering: `Manim Trial/README.md` |
...
| **D2L Assignment Assistant** | *(no standalone folder)* | AA code lives in `School Scrips\Macro App\assignment-assistant-engine\` (vendored engine). The old `Documents\D2L Assignment Assistant` workspace folder is gone as of 2026-09-07; only `%APPDATA%\D2L Assignment Assistant\` remains, and that is runtime config, not source. |
